...
export async function POST(req, context) {
  try {
    ...
    if (!name || !name.trim()) return NextResponse.json({ error: "Tên phòng không hợp lệ" }, { status: 400 });

    const cinema = await Cinema.findById(cinemaId);
    if (!cinema) return NextResponse.json({ error: "Cinema không tồn tại" }, { status: 404 });
